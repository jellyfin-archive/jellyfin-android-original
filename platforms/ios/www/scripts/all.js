...
window.appMode='cordova';window.dashboardVersion='3.0.5717.20922';
...
$(ownerpage).on('pagebeforeshowready',LibraryBrowser.onTabbedPageBeforeShowReady);$(pages).on('iron-select',function(){console.log('iron-select');var enablePaperTabs=LibraryBrowser.enableFullPaperTabs();var delay=enablePaperTabs?500:0;var pgs=this;setTimeout(function(){$(pgs).trigger('tabchange');},delay);});},onTabbedPageBeforeShowReady:function(){var page=this;var tabs=page.querySelector('paper-tabs');var selected=tabs.selected;if(selected==null){Logger.log('selected tab is null, checking query string');selected=parseInt(getParameterByName('tab')||'0');Logger.log('selected tab will be '+selected);if(LibraryBrowser.enableFullPaperTabs()){tabs.selected=selected;}else{page.querySelector('neon-animated-pages').selected=selected;}}else{Events.trigger(page.querySelector('neon-animated-pages'),'tabchange');}},canShare:function(item,user){return user.Policy.EnablePublicSharing;},getDateParamValue:function(date){function formatDigit(i){return i<10?"0"+i:i;}
