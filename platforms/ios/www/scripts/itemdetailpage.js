...
promise=promise||ApiClient.getItems(Dashboard.getCurrentUserId(),query);promise.done(function(result){var html='';if(item.Type=="MusicAlbum"){html=LibraryBrowser.getListViewHtml({items:result.Items,smallIcon:true,showIndex:true,index:'disc',showIndexNumber:true,playFromHere:true,defaultAction:'playallfromhere',lazy:true});}
else if(item.Type=="Series"){html=LibraryBrowser.getPosterViewHtml({items:result.Items,shape:"detailPagePortrait",showTitle:false,centerText:true,context:context,lazy:true});}
...
function removeFromCollection(page,parentItem,itemIds,user,context){Dashboard.showLoadingMsg();var url=ApiClient.getUrl("Collections/"+parentItem.Id+"/Items",{Ids:itemIds.join(',')});ApiClient.ajax({type:"DELETE",url:url}).done(function(){renderChildren(page,parentItem,user,context);Dashboard.hideLoadingMsg();});}
...
function renderCriticReviewsContent(page,result,limit){var html='';var reviews=result.Items;if(reviews.length){html+='<div class="paperList">';}
for(var i=0,length=reviews.length;i<length;i++){var review=reviews[i];html+='<paper-icon-item style="padding-top:.5em;padding-bottom:.5em;">';if(review.Score!=null){}
...
function renderAdditionalParts(page,item,user){ApiClient.getAdditionalVideoParts(user.Id,item.Id).done(function(result){if(result.Items.length){$('#additionalPartsCollapsible',page).show();$('#additionalPartsContent',page).html(getVideosHtml(result.Items,user)).lazyChildren();}else{$('#additionalPartsCollapsible',page).hide();}});}
...
var casts=item.People||[];casts=casts.filter(function(c){return c.PrimaryImageTag;});for(var i=0,length=casts.length;i<length;i++){var cast=casts[i];var href=isStatic?'#':'itemdetails.html?id='+cast.Id+'';html+='<div class="card '+getPortraitShape()+'Card">';html+='<div class="cardBox">';html+='<div class="cardScalable">';var imgUrl;var lazy=true;if(cast.PrimaryImageTag){imgUrl=ApiClient.getScaledImageUrl(cast.Id,{width:100,tag:cast.PrimaryImageTag,type:"primary",minScale:2});}else{imgUrl="css/images/items/list/person.png";lazy=false;}
html+='<div class="cardPadder"></div>';html+='<a class="cardContent" href="'+href+'">';if(lazy){html+='<div class="cardImage coveredCardImage lazy" data-src="'+imgUrl+'"></div>';}else{html+='<div class="cardImage" style="background-image:url(\''+imgUrl+'\');"></div>';}
...
$(document).on('pageinit',"#itemDetailPage",function(){var page=this;$('.btnPlay',page).on('click',function(){playCurrentItem(this);});$('.btnPlayTrailer',page).on('click',function(){playTrailer(page);});$('.btnSplitVersions',page).on('click',function(){splitVersions(page);});$('.btnSync',page).on('click',function(){SyncManager.showMenu({items:[currentItem]});});$('.btnRecord,.btnFloatingRecord',page).on('click',function(){var id=getParameterByName('id');Dashboard.navigate('livetvnewrecording.html?programid='+id);});$('.btnCancelRecording',page).on('click',function(){deleteTimer(page,currentItem.TimerId);});$('.btnMoreCommands',page).on('click',function(){var button=this;Dashboard.getCurrentUser().done(function(user){LibraryBrowser.showMoreCommands(button,currentItem.Id,LibraryBrowser.getMoreCommands(currentItem,user));});});$('.childrenItemsContainer',page).on('playallfromhere',function(e,index){LibraryBrowser.playAllFromHere(_childrenItemsFunction,index);}).on('queueallfromhere',function(e,index){LibraryBrowser.queueAllFromHere(_childrenItemsFunction,index);});$(page).on("click",".moreScenes",function(){Dashboard.getCurrentUser().done(function(user){renderScenes(page,currentItem,user);});}).on("click",".morePeople",function(){renderCast(page,currentItem,getContext(currentItem));}).on("click",".moreSpecials",function(){Dashboard.getCurrentUser().done(function(user){renderSpecials(page,currentItem,user);});}).on("click",".moreCriticReviews",function(){renderCriticReviews(page,currentItem);});}).on('pagebeforeshow',"#itemDetailPage",function(){var page=this;reload(page);Events.on(ApiClient,'websocketmessage',onWebSocketMessage);Events.on(LibraryBrowser,'itemdeleting',onItemDeleted);}).on('pagebeforehide',"#itemDetailPage",function(){Events.off(LibraryBrowser,'itemdeleting',onItemDeleted);currentItem=null;var page=this;Events.off(ApiClient,'websocketmessage',onWebSocketMessage);LibraryMenu.setTransparentMenu(false);});function itemDetailPage(){var self=this;self.play=play;self.setInitialCollapsibleState=setInitialCollapsibleState;self.renderDetails=renderDetails;self.renderCriticReviews=renderCriticReviews;self.renderCast=renderCast;self.renderScenes=renderScenes;self.renderMediaSources=renderMediaSources;}
