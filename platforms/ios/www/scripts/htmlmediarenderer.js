...
return null;};self.stop=function(){if(mediaElement){mediaElement.pause();if(isViblastStarted){requirejs(['https://viblast.com/player/free-version/sdqsdx86/viblast.js'],function(){viblast(mediaElement).stop();isViblastStarted=false;});}}};self.pause=function(){if(mediaElement){mediaElement.pause();}};self.unpause=function(){if(mediaElement){mediaElement.play();}};self.volume=function(val){if(mediaElement){if(val!=null){mediaElement.volume=val;return;}
return mediaElement.volume;}};var currentSrc;self.setCurrentSrc=function(val){var elem=mediaElement;if(!elem){currentSrc=null;return;}
...
else{if(enableViblast(val)){viblast(elem).setup({key:'N8FjNTQ3NDdhZqZhNGI5NWU5ZTI=',stream:val});isViblastStarted=true;}else{elem.src=val;$(elem).one("loadedmetadata",onLoadedMetadata);}}
currentSrc=val;};self.setTracks=function(tracks){var html=tracks.map(function(t){var defaultAttribute=t.isDefault?' default':'';return'<track kind="subtitles" src="'+t.url+'" srclang="'+t.language+'"'+defaultAttribute+'></track>';}).join('');if(html){mediaElement.innerHTML=html;}};self.currentSrc=function(){if(mediaElement){return currentSrc;}};self.paused=function(){if(mediaElement){return mediaElement.paused;}
...
if(elem.tagName.toLowerCase()!='audio'){$(elem).remove();}}};self.setPoster=function(url){var elem=mediaElement;if(elem){elem.poster=url;}};self.supportsTextTracks=function(){if(supportsTextTracks==null){supportsTextTracks=document.createElement('video').textTracks!=null;}
