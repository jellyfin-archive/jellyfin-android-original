...
return culture;}
function getDeviceCulture(){var deferred=DeferredBuilder.Deferred();if(navigator.globalization&&navigator.globalization.getPreferredLanguage){Logger.log('Calling navigator.globalization.getPreferredLanguage');navigator.globalization.getPreferredLanguage(function(locale){var culture=locale.value||'';culture=navigator.language||navigator.userLanguage||culture;Logger.log('Device culture is '+culture);deferred.resolveWith(null,[culture]);},function(){Logger.log('navigator.globalization.getPreferredLanguage failed');deferred.resolveWith(null,[navigator.language||navigator.userLanguage]);});}else if(AppInfo.supportsUserDisplayLanguageSetting){Logger.log('AppInfo.supportsUserDisplayLanguageSetting is true');deferred.resolveWith(null,[AppSettings.displayLanguage()]);}else{Logger.log('Getting culture from document');deferred.resolveWith(null,[document.documentElement.getAttribute('data-culture')]);}
return deferred.promise();}
