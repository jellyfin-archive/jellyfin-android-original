(function(){var supportsTextTracks;function htmlMediaRenderer(type){var mediaElement;var self=this;function onEnded(){$(self).trigger('ended');}
...
function createVideoElement(){var elem=$('.itemVideo');return elem.one('.loadedmetadata',onLoadedMetadata).one('playing',onOneVideoPlaying).on('timeupdate',onTimeUpdate).on('ended',onEnded).on('volumechange',onVolumeChange).on('play',onPlay).on('pause',onPause).on('playing',onPlaying).on('click',onClick).on('dblclick',onDblClick).on('error',onError)[0];}
...
else{$(elem).one("loadedmetadata",onLoadedMetadata);}};self.currentSrc=function(){if(mediaElement){return mediaElement.currentSrc;}};self.paused=function(){if(mediaElement){return mediaElement.paused;}
...
if(!window.VideoRenderer){window.VideoRenderer=htmlMediaRenderer;}})();