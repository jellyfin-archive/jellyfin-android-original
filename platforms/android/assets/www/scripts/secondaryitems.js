(function($,document){var data={};function addCurrentItemToQuery(query,item){if(item.Type=="Person"){query.PersonIds=item.Id;}
...
function reloadItems(page,parentItem){Dashboard.showLoadingMsg();var query=getQuery(parentItem);ApiClient.getItems(Dashboard.getCurrentUserId(),query).done(function(result){window.scrollTo(0,0);var html='';var pagingHtml=LibraryBrowser.getQueryPagingHtml({startIndex:query.StartIndex,limit:query.Limit,totalRecordCount:result.TotalRecordCount,showLimit:false});page.querySelector('.listTopPaging').innerHTML=pagingHtml;var trigger=false;if(query.IncludeItemTypes=="Audio"){html='<div style="max-width:1000px;margin:auto;">'+LibraryBrowser.getListViewHtml({items:result.Items,playFromHere:true,defaultAction:'playallfromhere',smallIcon:true})+'</div>';trigger=true;}else{var posterOptions={items:result.Items,shape:"auto",centerText:true,lazy:true,overlayText:true};if(query.IncludeItemTypes=="MusicAlbum"){posterOptions.overlayText=false;posterOptions.showParentTitle=true;posterOptions.overlayPlayButton=true;}
...
var elem=page.querySelector('#items');elem.innerHTML=html+pagingHtml;ImageLoader.lazyChildren(elem);if(trigger){$(elem).trigger('create');}
$('.btnNextPage',page).on('click',function(){query.StartIndex+=query.Limit;reloadItems(page,parentItem);});$('.btnPreviousPage',page).on('click',function(){query.StartIndex-=query.Limit;reloadItems(page,parentItem);});LibraryBrowser.setLastRefreshed(page);Dashboard.hideLoadingMsg();});}
$(document).on('pageinitdepends',"#secondaryItemsPage",function(){var page=this;$(page).on('click','.mediaItem',onListItemClick);}).on('pagebeforeshowready',"#secondaryItemsPage",function(){var page=this;if(getParameterByName('parentid')){ApiClient.getItem(Dashboard.getCurrentUserId(),getParameterByName('parentid')).done(function(parent){LibraryMenu.setTitle(parent.Name);if(LibraryBrowser.needsRefresh(page)){reloadItems(page,parent);}});}
else if(LibraryBrowser.needsRefresh(page)){reloadItems(page);}});})(jQuery,document);