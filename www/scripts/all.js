...
window.appMode='cordova';window.dashboardVersion='3.0.5688.23150';
...
return Number(version);};impl.isMobileFallback=function(userAgent){return impl.detectMobileBrowsers.fullPattern.test(userAgent)||impl.detectMobileBrowsers.shortPattern.test(userAgent.substr(0,4));};impl.prepareDetectionCache=function(cache,userAgent,maxPhoneWidth){if(cache.mobile!==undefined){return;}
...
jQuery.browser=browser;})(jQuery,window);(function($){})(jQuery);(function($){var $w=$(window);function visibleInViewport(elem,partial,hidden,direction,threshold){var $t=$(elem),t=elem,vpWidth=$w.width(),vpHeight=$w.height(),direction=(direction)?direction:'both',clientSize=hidden===true?t.offsetWidth*t.offsetHeight:true;if(typeof t.getBoundingClientRect==='function'){var rec=t.getBoundingClientRect(),tViz=rec.top>=0&&rec.top<vpHeight+threshold,bViz=rec.bottom>0&&rec.bottom<=vpHeight+threshold,lViz=rec.left>=0&&rec.left<vpWidth+threshold,rViz=rec.right>0&&rec.right<=vpWidth+threshold,vVisible=partial?tViz||bViz:tViz&&bViz,hVisible=partial?lViz||rViz:lViz&&rViz;if(direction==='both')
...
var unveilId=0;function getThreshold(){var screens=$.browser.mobile?2.5:1;return Math.max(screen.availHeight*screens,1000);}
var threshold=getThreshold();function isVisible(elem){return visibleInViewport(elem,true,false,'both',threshold);}
...
var isVlc=AppInfo.isNativeApp&&$.browser.android;var bitrateSetting=AppSettings.maxStreamingBitrate();if(isVlc){if((maxHeight||1080)>=1080){bitrateSetting=Math.min(bitrateSetting,4000002);}}
var canPlayWebm=self.canPlayWebm();var profile={};profile.MaxStreamingBitrate=bitrateSetting;profile.MaxStaticBitrate=4000000;profile.MusicStreamingTranscodingBitrate=Math.min(bitrateSetting,192000);profile.DirectPlayProfiles=[];if(canPlayH264()){profile.DirectPlayProfiles.push({Container:'mp4,m4v',Type:'Video',VideoCodec:'h264',AudioCodec:'aac,mp3'});}
