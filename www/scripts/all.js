...
window.appMode='cordova';window.dashboardVersion='3.0.5677.26416';
...
$(ownerpage).on('pagebeforeshowready',function(){LibraryBrowser.onTabbedPageBeforeShowReady(ownerpage,defaultTabIndex);});$(pages).on('iron-select',function(){var delay=LibraryBrowser.enableFullPaperTabs()?500:0;var pgs=this;setTimeout(function(){$(pgs).trigger('tabchange');},delay);});},onTabbedPageBeforeShowReady:function(page,defaultTabIndex){var tabs=page.querySelector('paper-tabs');var selected=tabs.selected;if(selected==null){Logger.log('selected tab is null, checking query string');if(!LibraryBrowser.enableFullPaperTabs()){defaultTabIndex=null;}
selected=parseInt(getParameterByName('tab'))||defaultTabIndex||0;Logger.log('selected tab will be '+selected);tabs.selected=selected;if(!LibraryBrowser.enableFullPaperTabs()){page.querySelector('neon-animated-pages').selected=selected;}}else{Events.trigger(page.querySelector('neon-animated-pages'),'tabchange');}},canShare:function(item,user){return user.Policy.EnablePublicSharing;},getDateParamValue:function(date){function formatDigit(i){return i<10?"0"+i:i;}
