...
function renderCollectionItemType(page,parentItem,type,items,user,context){var html='';html+='<div class="detailSection">';html+='<h1>';html+='<span>'+type.name+'</span>';html+='</h1>';html+='<div class="detailSectionContent">';var shape=type.type=='MusicAlbum'?'detailPageSquare':'detailPagePortrait';html+=LibraryBrowser.getPosterViewHtml({items:items,shape:shape,showTitle:true,centerText:true,context:context,lazy:true,showDetailsMenu:true,overlayMoreButton:true,showAddToCollection:false,showRemoveFromCollection:true});html+='</div>';html+='</div>';var collectionItems=page.querySelector('.collectionItems');$(collectionItems).append(html);ImageLoader.lazyChildren(collectionItems);$(collectionItems).off('removefromcollection').on('removefromcollection',function(e,itemId){removeFromCollection(page,parentItem,[itemId],user,context);});}
