...
function show(options){require(['paperbuttonstyle'],function(){var id='dlg'+new Date().getTime();var html='';html+='<paper-dialog id="'+id+'" entry-animation="scale-up-animation" exit-animation="fade-out-animation" with-backdrop>';if(options.title){html+='<h2>';html+=options.title;html+='</h2>';}
html+='<paper-dialog-scrollable>';for(var i=0,length=options.items.length;i<length;i++){var option=options.items[i];html+='<paper-button class="block blue ripple btnOption" data-id="'+option.id+'" style="margin:0;">';html+='<span>'+option.name+'</span>';html+='</paper-button>';}
